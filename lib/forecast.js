...
function fcWMA(series, h) {
  const w = qtyArr(series).slice(-3);
  const weights = [0.1, 0.3, 0.6].slice(3 - w.length);
...
function fcTrend(series, h) {
  const all = qtyArr(series);
  const isExtended = all.length > 24;          // BE appended to series
  // Slope from 6 completed months only (exclude synthetic BE)
  const y = isExtended ? all.slice(-7, -1) : all.slice(-6);
  const { a, b } = linregCoef(y);
  // Anchor: extrapolate from the last actual value (BE if present, else last completed)
  const anchor = all[all.length - 1];
  const out = [];
  for (let k = 1; k <= h; k++) out.push(clamp0(anchor + b * k));
  return out;
}
function fcSeasonal(series, h) {
  const q = qtyArr(series);
  if (q.length < 12) return fcTrend(series, h);
  ...
  const { a, b } = linregCoef(q.slice(-12));
...
function fcNewSku10(series, h) {
  const q = qtyArr(series).filter((v) => v > 0);
  const last = q.length ? q[q.length - 1] : 0;
  return Array(h).fill(clamp0(Math.round(last * 1.10)));
}
...
export const METHODS = {
  ...
  new_sku_10: { label: "New SKU (+10%)",   fn: fcNewSku10 },
};
export const METHOD_KEYS = ["naive", "ma3", "wma", "trend", "seasonal", "new_sku_10"];
...
export function champion(series, methods = METHOD_KEYS) {
  const activeMonths = series ? series.filter((p) => Number(p.q) > 0).length : 0;
  if (activeMonths < 3 && methods.includes("new_sku_10")) {
    return { method: "new_sku_10", wmape: null, nPred: 0 };
  }
  const scored = methods
    .map((m) => ({ method: m, ...backtest(series, m) }))
    .filter((x) => x.wmape !== null && x.nPred >= 3);
  if (!scored.length) {
    if (activeMonths < 3) return { method: "new_sku_10", wmape: null, nPred: 0 };
    return { method: "wma", wmape: null, nPred: 0 };
  }
