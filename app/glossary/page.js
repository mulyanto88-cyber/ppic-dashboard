...
const TERMS = [
  ...
  { group: "Forecast", term: "Best Estimate (BE) — current month", tag: "projection", def: "Projection of the in-progress month, shown with a * (e.g. Jul*). Month-to-date sales are grossed up by how far through the month we should be, where the progress % mirrors last month's weekly sales curve at the same day (day-weighted). This respects the end-of-month payday lift (from the 25th) instead of assuming sales land evenly across days. Display context only — it does not feed the forecast engine, the published baseline, or MPS/MRP.", formula: "BE = month-to-date ÷ progress% · progress% = last month's share of sales through the same day-of-month" },
