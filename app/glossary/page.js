...
const TERMS = [
  ...
  { group: "General", term: "Open PO / PO Pipeline", def: "Purchase order lines genuinely still running: Odoo status 'purchase' with outstanding quantity, shown for POs placed in the last 12 months (older history stays in the database, just not on screen). Force-closed lines are excluded from the pipeline and reported separately. Source: purchase.order export — PPIC scope only (RAW MATERIAL, FINISHED GOODS, SFG, CUKAI categories; marketing/capex/GA purchases are filtered out at ETL)." },
  ...
  { group: "General", term: "Force-closed PO (unfulfilled)", tag: "vendor signal", def: "PO lines closed while quantity was never delivered — the vendor could not fulfil. Tracked per vendor with the unfulfilled value as a reliability signal for sourcing decisions." },
